...
export default async function AdminSellersPage() {
  ...
  const sellers = JSON.parse(JSON.stringify(sellersRaw)).map((seller: any) => {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl bg-gradient-to-r from-gray-950 via-blue-950 to-gray-900 text-white p-5 md:p-8 shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
          ...
          <Link
            href="/become-seller"
            className="bg-yellow-400 text-black px-5 py-3 rounded-2xl font-extrabold text-center"
          >
            View Seller Landing
          </Link>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl border overflow-hidden">
        <div className="p-5 border-b">
          <h2 className="text-2xl font-extrabold">Seller Requests</h2>
              ...
              {sellers.length === 0 ? (
                ...
                sellers.map((seller: any) => (
                  <tr key={seller.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-4">
                      <p className="font-bold">{seller.total_products}</p>
                      <p className="text-xs text-gray-500">
                        Approved: {seller.approved_products}
                      </p>
                    </td>
                    ...
                    <td className="p-4">
                      <p className="font-bold">PAN: {seller.pan || seller.pan_number || "-"}</p>
                      <p className="text-xs text-gray-500">
                        GST: {seller.gst || seller.gst_number || "Optional"}
                      </p>
                    </td>
...
function Stat({
  ...
  return (
    <div className="bg-white rounded-2xl shadow border p-4 md:p-5">
      ...
      <h2 className={`text-2xl md:text-3xl font-extrabold mt-1 ${color}`}>
        {value}
      </h2>
    </div>
  );
